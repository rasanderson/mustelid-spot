"""
Test set extraction utility for reproducible model evaluation.

Extracts the exact same test split that would be generated by the main training 
pipeline's load_dataset function, ensuring consistent evaluation across different 
model versions and experiments.

Key Features:
- Replicates exact train/test split logic from training pipeline
- Maintains reproducibility with fixed random seeds
- Handles class balancing (samples min class count from each)
- Supports multi-class dataset with fox vs 6 other species
- Outputs test set paths with labels for external evaluation tools

Dataset Structure Expected:
- base_dir/Fox/ (fox images, class 0)
- base_dir/Not Fox/preprocessed/[class_folders]/ (other species, classes 1-6)

Output: tab-delimited file with image paths and class labels for use with 
evaluation scripts and visualization tools.

Usage: Update base_directory path, run to generate test_set_paths_data_v16.txt
Essential for consistent model comparison and performance tracking.
"""
import random
import numpy as np
