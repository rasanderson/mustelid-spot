...
def main():
    ...
    if classification_model and os.path.exists(animal_crops_dir) and os.listdir(animal_crops_dir):
        ...
        try:
            ...
            crop_files = list(results_df[image_col])
            labels = list(results_df[class_col])
            confidences = list(results_df[conf_col])
            
            # Calculate how many grids we need (each grid has 16 images)
            total_images = len(crop_files)
            images_per_grid = 16
            ...
            for grid_num in range(num_grids):
                ...
                grid_crop_files = crop_files[start_idx:end_idx]
                grid_labels = labels[start_idx:end_idx]
                grid_confidences = confidences[start_idx:end_idx]
                ...
                for i, (crop_file, label, confidence) in enumerate(zip(grid_crop_files, grid_labels, grid_confidences)):
                    if i >= len(axes):
                        break
                    ...
                    try:
                        ...
                        axes[i].imshow(img)
                        # Add sequential number to the title
                        img_num = start_idx + i + 1  # +1 to start from 1 instead of 0
                        axes[i].set_title(f"#{img_num}: {label}\n{confidence:.2f}", fontsize=10)
                        axes[i].axis('off')
                    except Exception as e:
                        print(f"Error loading image {full_path}: {e}")
                        img_num = start_idx + i + 1
                        axes[i].text(0.5, 0.5, f"#{img_num}: Error loading\n{os.path.basename(crop_file)}", 
                                    ha='center', va='center')
                        axes[i].axis('off')
        
                # Turn off any unused subplots
                for j in range(len(grid_crop_files), len(axes)):
                    axes[j].axis('off')
        
                # Save the grid visualization with sequential numbering
                plt.tight_layout()
                grid_output = os.path.join(output_dir, f'animal_classification_grid_{grid_num+1}.png')
                plt.savefig(grid_output, dpi=200)
                plt.close(fig)
                print(f"Grid visualization {grid_num+1} saved to {grid_output}")
